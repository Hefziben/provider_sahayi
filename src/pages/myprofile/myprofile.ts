...
export class MyprofilePage {
  ...
  private user: User;
  private location = new MyLocation();
  ...
  constructor(private navCtrl: NavController, private service: ClientService,
    ...
    let location = JSON.parse(window.localStorage.getItem(Constants.KEY_LOCATION));
  ...
  ionViewDidEnter() {
    ...
    if (newSelectedLocation) this.location = newSelectedLocation;
  ...
  refreshProfile() {
    let subscription: Subscription = this.service.getProfile(window.localStorage.getItem(Constants.KEY_TOKEN)).subscribe(res => {
      if (res && res.primary_category && res.primary_category_id) {
        this.profile = res;
		if (this.profile.latitude && this.profile.latitude.length && this.profile.longitude && this.profile.longitude.length) {
          this.location = new MyLocation();
          this.location.name = this.profile.address;
          this.location.lat = this.profile.latitude;
          this.location.lng = this.profile.longitude;
        }
