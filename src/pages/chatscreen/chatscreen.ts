...
export class ChatscreenPage {
  ...
  notifyMessages() {
    this.clientService.postNotification(window.localStorage.getItem(Constants.KEY_TOKEN), "customer", Number(this.chat.chatId) ? this.chat.chatId : this.chat.chatId.substring(0, this.chat.chatId.indexOf("hc"))).subscribe(res => console.log("notiS", res), err => console.log("notiF", err));
  }
